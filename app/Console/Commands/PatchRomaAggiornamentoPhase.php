...
class PatchRomaAggiornamentoPhase extends Command
{
    protected $signature = 'works:patch-roma-aggiornamento-phase
        {--apply : Persist the work_phase_id change (omit for dry-run)}
        {--force : Skip the production confirmation prompt}';

    protected $description = 'Set work_phase_id = 10 on AGGIORNAMENTO works (phase 3) whose central is in ROMA with network_scope_id 1 or 2.';

    private const SOURCE_PHASE = 3;
    private const TARGET_PHASE = 10;
    private const REGION        = 'ROMA';
